							...
							)
{
	...
	const LDC_MODEL_DTYPE* __restrict__ f = LDC_MODEL_F;
	const LDC_MODEL_DTYPE* __restrict__ v = LDC_MODEL_V;
	const LDC_MODEL_DTYPE* __restrict__ c = LDC_MODEL_C;
	...
	for (i=0; i<num_feature; i++)
	{
		...
		const LDC_MODEL_DTYPE* __restrict__ c_ptr = c;
		for (j=0; j<fhv_dim_word; j++)
		{
			const LDC_MODEL_DTYPE vf = v_val ^ *f++;
			for (k=0; k<num_class; k++)
	...
	for (k=0; k<num_class; k++)
	{
		const LDC_MODEL_DTYPE* __restrict__ f_ptr = f;
		for (i=0; i<num_feature; i++)
		{
			LDC_MODEL_DTYPE v_val = v[in[i]];
			const LDC_MODEL_DTYPE* __restrict__ c_ptr = &c[k * fhv_dim_word];
			for (j=0; j<fhv_dim_word; j++)
			{
...
uint8_t ldc_inference_compute(const uint8_t in[], const uint16_t in_length
#ifndef LDC_IMPLEMENTATION_FIX_SIZE
							, const LDC_MODEL_DTYPE* __restrict__ f, const LDC_MODEL_DTYPE* __restrict__ v, const LDC_MODEL_DTYPE* __restrict__ c
							, uint8_t num_class, uint16_t num_feature, uint8_t fhv_dim_word
#endif
